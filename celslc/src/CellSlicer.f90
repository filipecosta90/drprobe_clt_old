...
!
!!   maximum value of the last slice potential imaginary part
!  real*4, public :: CS_lastslice_potmax_im
!  DATA CS_lastslice_potmax_im /0.0/
!
...
!        if (g>apthr) cycle ! scattering amplitude aperture
        ...
        if (CS_doconsolemsg>0) then
          ! update progress indicator
          if (npc < int( 20.0*real(ncalc)/real(ncalcmax) ) ) then
            npc = npc + 1
            write(unit=6,fmt='(A,$)') "."
          end if
        end if
